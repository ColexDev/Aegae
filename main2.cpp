...
int main();
void menu_initilization(const std::vector<std::string> &arrChoice, int direction, int xStarxStart, int yStart);
void find_entry();
void addEntry();
void filter_results();
...
void menu_initilization(const std::vector<std::string> &par_ARRCHOICE, int direction, int xStart, int yStart)
{
...
void viewEntry()
{
    ...
    while(true) {
        ...
        menu_initilization(CHOICESVIEWENTRY, 1, 2, 1);
        if(keyPress == 108) {
            switch (highlight) {
                case 0:
                    clearRefresh();
...
/* SEPERATE THIS INTO MANY FUNCTIONS */
void getSetAmount()
{
...
/* make the 2 below function into 1 functon. 2 paramters (1. the text to print out, 2. The type) */
void setCategoryIncome()
{
    ...
    while(true) {
        ...
        menu_initilization(CHOICESCATEGORYINCOME, 1, 2, 1);
        switch(keyPress) {
            case KEY_L:
...
void setCategoryExpense()
{
    ...
    while(true) {
        ...
        menu_initilization(CHOICESCATEGORYEXPENSE, 1, 2, 1);
        switch(keyPress) {
            case KEY_L:
...
void addEntry()
{
    ...
    while(true) {
        ...
        menu_initilization(CHOICESTYPE, 1, 2, 1);
        switch(keyPress) {
            case 108:
                clearRefresh();
...
float calculate_money_left_over(std::vector<std::string> &vec)
{
    float amount = 0;
    for (auto entry : vec) {
...
/* REMOVE NUMENTRIES FROM EXISTANCE, USE SIZE OF VECTOR INSTEAD */
/* FIX CALCULATIONS OF AMOUNT */
void setup_menu(std::vector<std::string> &vec)
{
    ...
    ss << calculate_money_left_over(vec);
    std::string amountString(ss.str());
    std::string print = "Total money left over: $" + amountString;
    attron(A_BOLD);
    mvprintw(vec.size() + 5, 1, print.c_str());
    attroff(A_BOLD);
    while(true) {
        curs_set(0);
        menu_initilization(vec, 1, 4, 0);
        switch (keyPress) {
        case 110:
            clearRefresh();
            addEntry();
...
void filter_results()
{
    ...
    highlight = 0;
    while (true) {
        menu_initilization(months, VERTICAL, 2, 1);
        switch (keyPress) {
        case KEY_L:
            month = highlight + 1;
            ...
            calculate_money_left_over(specificMonthEntries);
